...
(function (global) {
  ...
  ───────────────────────────────────────── */
  const CFG = {
    // Page de vérification au chargement
    gatewayEnabled: true,        // activer la page style Cloudflare
    gatewayTTL: 3_600_000,       // durée du pass (1h) avant re-vérification
    siteName: location.hostname, // nom affiché sur la page
    powDifficulty: 3,            // difficulté PoW gateway (1-4)
  ...
  /* ─────────────────────────────────────────
     GATEWAY — Page de vérification style Cloudflare
  ───────────────────────────────────────── */
  const Gateway = {
    _overlay: null,
    ...
    _injectStyles() {
      ...
      s.textContent = `
        ...
        ._sh_title {
          font-size:21px;font-weight:700;color:#f8fafc;margin-bottom:6px;line-height:1.3;
        }

        ._sh_sub {
          font-size:13.5px;color:#64748b;margin-bottom:30px;line-height:1.65;
        }

        ._sh_progress_wrap {
          background:#1a1a28;border-radius:999px;height:5px;overflow:hidden;margin-bottom:14px;
        }
        ._sh_progress_bar {
        ...
        ._sh_status {
          font-size:12.5px;color:#94a3b8;min-height:18px;margin-bottom:24px;
          display:flex;align-items:center;justify-content:center;gap:7px;
        }
        ._sh_dot {
          width:5px;height:5px;border-radius:50%;background:#6366f1;flex-shrink:0;
          animation:_sh_pulse 1.1s ease-in-out infinite;
        }
        @keyframes _sh_pulse {
        ...
        ._sh_checks {
          display:flex;flex-direction:column;gap:8px;margin-bottom:26px;text-align:left;
        }
        ._sh_check {
  ...
  const Shield = {
    ...
    RateLimit, BotDetect, Honeypot, Gateway,
    config: CFG,
  };
